...
function findFirstSingleChar(str) {
  ...
  const result = Object.values(obj).indexOf(1);
  if (result !== -1) {
...
function reverseString(str) {
  return str.split(' ').map((element) => [...element].reverse().join('')).reverse().join(' ');
}
...
function isCreditCardNumber(ccn) {
  let sum = 0;
  [...`${ccn}`].reverse().forEach((element, i) => {
    let result = 0;
    if ((i + 1) % 2 === 0) {
      result = element * 2;
      if (result > 9) {
        result = [...`${result}`].reduce((summ, current) => Number(summ) + Number(current));
      }
    } else {
      result = element;
    }
...
function isBracketsBalanced(str) {
  ...
  let re;
  for (const i in text) {
    for (const value of bracketsConfig) {
